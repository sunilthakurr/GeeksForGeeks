...
class Solution {
    ...
    public int inorderSuccessor(Node root, Node x) {
        // add code here.
        Node suc = new Node(Integer.MAX_VALUE);
        Node successor = solve(root, x, suc);
        return successor.data == Integer.MAX_VALUE ? -1: successor.data;
        
        
    }
    public Node solve(Node root, Node x, Node successor) {
        ...
        if(x.data < root.data) {
            ...
            return solve(root.left, x, successor);
        }
        else {
            return solve(root.right, x, successor);
        }
    }
}